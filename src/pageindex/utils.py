...
def extract_doc_date(cover_text, model=None):
    prompt = f"""You are an expert in reading document cover pages.
    You are given the text of a document's first page (cover page). Your task is to
    extract the document's date (e.g. issue date, publication date, decision date)
    and return it in YYYY-MM-DD format. If the day is unknown, return YYYY-MM.
    If there is no clear date on this page, return an empty string.
